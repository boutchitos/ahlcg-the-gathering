...
export const DEFAULT_PLAYER_CARDS_SORTING_ORDER = Object.keys(PlayerCardSorter).filter((v) => isNaN(Number(v))) as PLAYER_CARDS_SORTER[];

export function fixPlayerCardsSortingOrder(wannaBe: string[]): PLAYER_CARDS_SORTER[] {
  const incoming = new Set(wannaBe.filter((sorter) => DEFAULT_PLAYER_CARDS_SORTING_ORDER.includes(sorter as PLAYER_CARDS_SORTER)));
